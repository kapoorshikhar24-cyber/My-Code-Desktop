...
set "PASS_HASH=240be518fabd2724ddb6f04eeb1da5967448d7e831c08c8fa822809f74c720a9"
echo ==================================================
echo             ENTER PASSWORD TO ACCESS
echo ==================================================
powershell -Command "$p = Read-Host -AsSecureString 'Password'; if (-not $p) { exit 1 }; $b = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($p); $plain = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($b); $hash = [BitConverter]::ToString((New-Object System.Security.Cryptography.SHA256Managed).ComputeHash([System.Text.Encoding]::UTF8.GetBytes($plain))).Replace('-', '').ToLower(); if ($hash -ne $env:PASS_HASH) { exit 1 }"
if %errorlevel% neq 0 (
...
:: 6. Run the script
echo.
choice /c yn /m "Do you want to run the switcher in hidden background mode"
if %errorlevel% equ 1 (
    echo Starting in hidden background mode...
    start "" pythonw .venv\mouse_switcher.py
    echo.
    echo [*] Switcher is now running in the background!
    echo [*] To stop it, run 'stop_mouse_switcher.bat'.
    ping 127.0.0.1 -n 4 >nul
    exit /b
) else (
    echo Starting in normal windowed mode...
